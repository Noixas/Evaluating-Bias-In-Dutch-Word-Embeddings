...
class Relation:
    ...
    def test_model_2020(self, model): #modern version using keyed_vectors gensim
        """
        Tests a given model with the set.
        ...
        # The most_similar assignment is neccessary because the most_similar function might refer to the original
        # Word2Vec function.
        return model.evaluate_word_analogies(self.pathtoset)
    @staticmethod
...
if __name__ == "__main__":
    ...
    cats = json.load(open("WEAT_clips/data/semtest.json"))

    # Create the relation set tuples, and saves the result to question-words.txt
    Relation.create_set(cats, "WEAT_clips/data/question-words.txt")
    
    pathtomodel = ""
    model = Word2Vec.load_word2vec_format(pathtomodel)

    rel = Relation("WEAT_clips/data/question-words.txt")
