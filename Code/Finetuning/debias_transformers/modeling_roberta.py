...
ROBERTA_PRETRAINED_MODEL_ARCHIVE_LIST = [
    ...
    # See all RoBERTa models at
    # https://huggingface.co/models?filter=roberta
]
    ...
    def forward(self, input_ids=None, token_type_ids=None,
        ...
        if position_ids is None:
            if input_ids is not None:
                # Create the position ids from the input token ids. Any
                # padded tokens remain padded.
...
class RobertaForSequenceClassification(BertPreTrainedModel):
    ...
    def forward(
        ...
        if (True):
            ...
            # bias_dir = torch.tensor(bias_dir,device=input_ids.device)
            ...
            if (remove_bias):
                ...
                embeddings = torch.nn.functional.normalize(embeddings, p=2, dim=-1)
            sequence_output = embeddings #back to transformers naming
			# # average over time steps
			# embeddings = torch.mean(embeddings, dim=1) # [batch size x hidden
			# size]
        # else: #when we use pooled_output
