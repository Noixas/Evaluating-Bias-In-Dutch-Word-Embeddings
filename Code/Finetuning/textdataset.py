...
class TextDataset(Dataset):
    def __init__(self, tokenizer, model_name_or_path, file_path="../../Rodrigo-data/Finetuning/dbrd/train",
                 block_size=512, overwrite_cache=True, mask_padding_with_zero=True):
        print(file_path)
        ...
        cached_features_file = os.path.join(
            directory, model_name_or_path + "_cached_lm_" +
                ...
                with open(file_path + ".sentences.txt", encoding="utf-8") as f:
                    for sentence in f:
                        ...
                        if len(tokenized_text) is 0: #avoid sentence that couldnt be tokenized in bertje
                            label = next(flabel) # we are skipping the sentence so should skip the label too.
                            continue
                        ...
                        input_mask = [
                            1 if mask_padding_with_zero else 0] * len(tokenized_text)
                        ...
                        label = next(flabel) #FUCK YOU LINE OF CODE. 
                        self.examples.append([tokenized_text[0: block_size -
                                                             3], input_mask[0: block_size -
                                                                            3], [1] if label.startswith("1") else [0]])
    ...
    def __getitem__(self, item):
        return [torch.tensor(self.examples[item][0]), torch.tensor(
            self.examples[item][1]), torch.tensor([0]), torch.tensor(self.examples[item][2])]
