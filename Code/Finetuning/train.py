...
class Train:
    def train(args, train_dataset, model, tokenizer,
              evaluate_fn=None, debias=False, bias_dir=None):
        """ Train the model """
        ...
        # Distributed training (should be after apex fp16
        # initialization)
        ...
        for _ in train_iterator:
            ...
            for step, batch in enumerate(epoch_iterator):
                ...
                # Skip past any already trained steps if resuming
                # training
                ...
                inputs = {
                    ...
                    "labels": batch[3],
                    "remove_bias":debias,
                    "bias_dir":bias_dir} if debias else {
                    "input_ids": batch[0],
                    "attention_mask": batch[1],
                    "labels": batch[3]}
                ...
                # model outputs are always tuple in transformers (see
                # doc)
                loss = outputs[0]
                ...
                if (step + 1) % args.gradient_accumulation_steps == 0:
                    ...
                    if args.local_rank in [
                        ...
                        if (
                            ...
                            results = evaluate(args, model, tokenizer,debias, bias_dir)
            ...
            if evaluate_fn is not None:
                results = pd.DataFrame(evaluate_fn(
                    args.evaluate_dataset, model,debias, bias_dir))
                cm = ConfusionMatrix(
                    actual_vector=results['true'].values,
                    predict_vector=results['predicted'].values)
                logs = {}
